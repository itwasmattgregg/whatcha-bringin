...
import { Gathering, Item } from '../types';

export const gatheringsService = {
  getGatherings: async (): Promise<Gathering[]> => {
  ...
  updateGathering: async (id: string, data: {
    name?: string;
    date?: string;
    time?: string;
    address?: string;
  ...
  addItem: async (gatheringId: string, data: { name: string; type: 'food' | 'drink' }): Promise<Item> => {
    const response = await apiClient.post(`/gatherings/${gatheringId}/items`, data);
    return response.data;
  },
  
  claimItem: async (gatheringId: string, itemId: string, name: string, customDescription?: string): Promise<Item> => {
    const response = await apiClient.post(`/gatherings/${gatheringId}/claim-item`, {
      itemId,
      name,
      customDescription,
    });
  ...
  getInviteShare: async (gatheringId: string): Promise<{ code: string; link: string; message: string }> => {
