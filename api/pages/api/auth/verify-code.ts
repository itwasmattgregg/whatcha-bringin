...
export default async function handler(
  ...
  try {
    ...
    if (isTestAccount && isTestCodeMatch) {
      console.log(
        `[TEST ACCOUNT] Test code accepted for test account ${normalizedPhone}`
      );
      isVerified = true;
    } else if (isDevelopment && isDevTestNumber && isDevTestCode) {
      console.log(
        `[DEV MODE] Test code accepted for test number ${normalizedPhone}`
      );
      isVerified = true;
    } else {
      ...
      try {
        ...
        const twilioError = verifyError as {
          code?: number;
          message?: string;
          status?: number;
        };
